...
variable "service_connectors_configuration" {
  description = "Service Connector configuration settings, defining all aspects to manage service connectors and related resources in OCI. Please see the comments within each attribute for details."
  ...
  type = object({
    ...
    service_connectors = map(object({
      ...
      source = object({
        kind = string # default is "logging". No other kind is currently supported.
      ...
      target = object({ # the target
        kind = string, # supported targets: "objectstorage", "streaming", "functions", "logginganalytics".
