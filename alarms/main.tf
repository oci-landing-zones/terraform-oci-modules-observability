...
resource "oci_monitoring_alarm" "these" {
  ...
  repeat_notification_duration = each.value.supplied_alarm != null ? each.value.supplied_alarm.severity != null ? (each.value.supplied_alarm.severity == "CRITICAL" ? (each.value.supplied_alarm.repeat_frequency_for_critical_alarms != null ? each.value.supplied_alarm.repeat_frequency_for_critical_alarms : "PT4H") : each.value.supplied_alarm.repeat_frequency_for_critical_alarms) : "PT4H" : local.preconfigured_alarms[each.value.preconfigured_alarm_type].repeat_frequency_for_critical_alarms
