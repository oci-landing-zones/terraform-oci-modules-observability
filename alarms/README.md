# OCI Landing Zone Alarms Module
...
Within the *alarms* attribute, the alarms to trigger can be defined in two ways: through pre-configured alarm types or by supplying specific alarms.
- **pre-configured alarm types**: use the *preconfigured_alarm_type* attribute, assigning it a list containing one or more of the following values:
  ...
  - **Exadata VM Cluster**: *EXADATA-VM-CLUSTER-HIGH-CPU-ALARM-WARNING*, *EXADATA-VM-CLUSTER-HIGH-CPU-ALARM-CRITICAL*, *EXADATA-VM-CLUSTER-HIGH-MEMORY-ALARM-WARNING*, *EXADATA-VM-CLUSTER-HIGH-MEMORY-ALARM-CRITICAL*, *EXADATA-VM-CLUSTER-HIGH-FILESYSTEM-UTILIZATION-ALARM-WARNING*, *EXADATA-VM-CLUSTER-HIGH-FILESYSTEM-UTILIZATION-ALARM-CRITICAL*, *EXADATA-VM-CLUSTER-HIGH-ASM-DISKGROUP-UTILIZATION-ALARM-WARNING*, *EXADATA-VM-CLUSTER-HIGH-ASM-DISKGROUP-UTILIZATION-ALARM-CRITICAL*, *EXADATA-VM-CLUSTER-HIGH-SWAP-UTILIZATION-ALARM-WARNING*, *EXADATA-VM-CLUSTER-HIGH-SWAP-UTILIZATION-ALARM-CRITICAL*, and *EXADATA-VM-CLUSTER-NODE-STATUS-ALARM-CRITICAL*.
  - **Exadata Database**: *EXADATA-DATABASE-HIGH-CPU-ALARM-WARNING*, *EXADATA-DATABASE-HIGH-CPU-ALARM-CRITICAL*, *EXADATA-DATABASE-HIGH-STORAGE-UTILIZATION-ALARM-WARNING*, and *EXADATA-DATABASE-HIGH-STORAGE-UTILIZATION-ALARM-CRITICAL*.

  The legacy lowercase types *high-cpu-alarm*, *instance-status-alarm*, *vm-maintenance-alarm*, *bare-metal-unhealthy-alarm*, *high-memory-alarm*, *adb-cpu-alarm*, *adb-storage-alarm*, *vpn-status-alarm*, and *fast-connect-status-alarm* remain available but are deprecated. For the metrics and thresholds used by each type, see [preconfigured_alarms.tf](./preconfigured_alarms.tf).
