...
variable "events_configuration" {
  ...
  type = object({
    ...
    event_rules = map(object({
      ...
      is_enabled = optional(bool) # whether the rule should be enabled. Default is false.
      ...
      supplied_events = optional(list(string)) # your own list of event type names. It has precedence over preconfigured event conditions denoted by preconfigured_events_categories setting.
      event_display_name = optional(string) # your event display name.
      event_description = optional(string) # your event description name.
    ...
    streams = optional(map(object({ # the streams to manage in this configuration.
      ...
      partitions = optional(string) # the number of stream partitions. Default is "1"  
      retention_in_hours = optional(string) # for how long to keep messages in the stream. Default is "24" hours
