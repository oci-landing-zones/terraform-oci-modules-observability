  ...
  type = object({
    ...
    event_rules = map(object({
      ...
      attributes_filter = optional(list(object({ # a list of attribute filters to restrict which events are captured.
        ...
        attr_in_additionalDetails_elem = optional(bool) # whether the attribute is expected to be found under additionalDetails element in event payload. This is service specific.
      }))) 
